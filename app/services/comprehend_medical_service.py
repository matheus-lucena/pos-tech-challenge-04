...
class ComprehendMedicalService:
    ...
    @staticmethod
    def _group_entities_by_type(entities: list) -> Dict[str, List[Dict[str, Any]]]:
        """Group a list of Comprehend Medical entity dicts by their Type field."""
    ...
    def detect_entities(self, text: str) -> Dict[str, Any]:
        try:
            ...
            entities_by_type = self._group_entities_by_type(response.get("Entities", []))
            return {
    ...
    def detect_phi(self, text: str) -> Dict[str, Any]:
        try:
            ...
            phi_by_type = self._group_entities_by_type(response.get("Entities", []))
            return {
                "phi": phi_by_type,
    ...
    def format_analysis_result(self, analysis: Dict[str, Any]) -> str:
        ...
        lines.append("\n📊 SUMMARY:")
        lines.append(
            f"   - Entity types: {', '.join(analysis['summary']['entity_types']) or 'None'}"
        )
        lines.append(
            f"   - PHI types: {', '.join(analysis['summary']['phi_types']) or 'None'}"
        )
        lines.append(
            f"   - Contains medical info: {'Yes' if analysis['summary']['has_medical_info'] else 'No'}"
        )
        lines.append(
            f"   - Contains PHI: {'Yes' if analysis['summary']['has_phi'] else 'No'}"
        )
